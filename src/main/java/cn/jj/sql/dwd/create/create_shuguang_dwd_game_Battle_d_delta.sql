...
--     维度
    ...
    match_elo               INT COMMENT '对局平均elo分（异常对局会存在多个team_elo，取平均得到match_elo）',
    pool_id                 STRING COMMENT '匹配池编号（一场对局可能会对应多个pool_id，字符串拼接，分割符为“,”）',
...
--     duration                INT COMMENT '匹配持续时间',
    ...
    server                  string comment '服务器名（一场对局可能会对应多个server，字符串拼接，分割符为“,”）',
--     度量
