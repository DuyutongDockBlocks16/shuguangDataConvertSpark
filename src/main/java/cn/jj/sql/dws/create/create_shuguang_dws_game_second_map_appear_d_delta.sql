...
-- 命名规范：小驼峰，关键字后面+"_col"
-- 属性注释参照来源表
drop table if exists shuguang.shuguang_dws_game_second_map_appear_d_delta;
CREATE TABLE IF NOT EXISTS shuguang.shuguang_dws_game_second_map_appear_d_delta
(
    battle_id           BIGINT,
--     主键
    map_area_id                INT,
...
--     度量
    ...
    COMMENT '描述地图不同区域出现次数。数据源为shuguang_dws_game_frame_map_appear_d_delta。主体为地图区域，粒度为每秒、每个分段、每个模式、每个英雄、每个职业、每个地图区域，对所有对局的出现次数做聚合。可以按照帧号、阵营、玩家类型、英雄、职业、业务区域分类进行筛选'
