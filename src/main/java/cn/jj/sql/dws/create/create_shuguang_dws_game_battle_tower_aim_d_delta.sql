...
-- 	展示方式：报表
-- 	问题：目前数据支撑不了伤害的统计；
drop table if exists shuguang.shuguang_dws_game_battle_tower_aim_d_delta;
CREATE TABLE IF NOT EXISTS shuguang.shuguang_dws_game_battle_tower_aim_d_delta
(
    -- 主键（自然键）
...
--     tower end
    ...
    hp_section          STRING,
    mana_section          STRING,
    aim_count           INT COMMENT '瞄准次数。'
)
    COMMENT '描述每局每个英雄被塔瞄准的次数和。数据源为shuguang_dws_game_frame_tower_aim_d_delta。主体为英雄，粒度为每局、每个分段、每个模式、每个英雄、每个职业。可以按照阵营、玩家类型、英雄、职业、防御塔类型、hp/mana区间进行筛选'
