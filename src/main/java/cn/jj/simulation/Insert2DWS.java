...
//    public static String[] RETURN_TYPE = {"Hero","Creep","Monster","Tower","Error"};
    ...
    public static void main(String[] args) throws Exception {
        ...
        int hosting_minute_section_interval = Integer.valueOf(args[7]);

        System.out.println("---- hosting_minute_section_interval: "+hosting_minute_section_interval);
...
//            System.out.println("---- mana: "+lines[13]+", mana max: "+lines[14]+", mana_percent: "+mana_percent);
            ...
            transObj.put("player_type", lines[80]);
...
//        map_appear 业务
        if ("1".equals(dws_model) || "5".equals(dws_model)) {
            hero_state_rich_RDD.mapToPair(hero_state_rich -> {
                ...
                transObj.put("player_type", richObj.get("player_type").toString());
                transObj.put("appear", richObj.get("appear").toString());
                String playerBattle = bc.value().get("playerBattle").get(richObj.get("battle_id").toString() + ":" + richObj.get("hero_id").toString() + ":" + richObj.get("camp_id").toString());
                ...
                String groupby_key = transObj.get("battle_id").toString()
                        ...
                        + HiveUtils.FIELDS_TERMINATED + transObj.get("player_type").toString();
...
//        tower_aim 业务
        if ("2".equals(dws_model) || "5".equals(dws_model)) {
            ...
            JavaPairRDD<String, String> hero_state_rich_pairRDD = hero_state_rich_RDD.mapToPair(hero_state_rich -> {
                ...
                transObj.put("player_type", richObj.get("player_type"));
            ...
            join_RDD.mapToPair(join -> {
                ...
                String playerBattle = bc.value().get("playerBattle").get(battle_id + ":" + richObj.get("hero_id").toString() + ":" + richObj.get("camp_id").toString());
                if (StringUtils.isEmpty(playerBattle)) {
                    transObj.put("ai_type", "");
                    transObj.put("ai_base", "");
                    transObj.put("deepainum", "");
        ...
        // tp业务
        if ("3".equals(dws_model) || "5".equals(dws_model)) {
            hero_state_rich_RDD.mapToPair(hero_state_rich -> {
                JSONObject richObj = JSONUtil.JSONString2JSONObject(hero_state_rich);
                JSONObject transObj = new JSONObject();
                transObj.put("battle_id", richObj.get("battle_id").toString());
        ...
        if ("4".equals(dws_model) || "5".equals(dws_model)) {
            // 过滤掉机器玩家，机器玩家不属于真人，也不存在托管行为
            JavaRDD<String> hero_state_rich_filter_RDD = hero_state_rich_RDD.filter(hero_state_rich -> {
                JSONObject richObj = JSONUtil.JSONString2JSONObject(hero_state_rich);
                String player_type = richObj.get("player_type").toString();
                if ("0".equals(player_type)) {
                    return true;
                } else {
                    return false;
                }
            });
            JavaPairRDD<String, String> hero_state_rich_pairRDD1 = hero_state_rich_filter_RDD.mapToPair(hero_state_rich -> {
                JSONObject richObj = JSONUtil.JSONString2JSONObject(hero_state_rich);
                JSONObject transObj = new JSONObject();
                transObj.put("race_id", richObj.get("race_id"));
            ...
            JavaPairRDD<String, String> hero_state_rich_pairRDD2 = hero_state_rich_filter_RDD.mapToPair(hero_state_rich -> {
                JSONObject richObj = JSONUtil.JSONString2JSONObject(hero_state_rich);
                JSONObject transObj = new JSONObject();
                transObj.put("kill", richObj.get("kill"));
                transObj.put("assist", richObj.get("assist"));
...
//                System.out.println("---- type1: "+userArenaEndStat_deepai);
//                System.out.println("---- type2: "+userArenaEndStat_strategyai);
